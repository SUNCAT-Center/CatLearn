    ...
    def hierarcy(self, features, min_split, max_split, hv, new_data=True,
        ...
        for indicies in reversed(index_split):
            ...
            if featselect_featvar:
                (p_error_select, number_feat, index2,
                 selected_features) = PH.predict_subsets(
                ...
                if featselect_featconst:
                    (data_size, p_error, coef, reg_data, result,
                     reg_store, selected_features, no_feature)\
                     = PH.predict_subsets(data_size=data_size,
                                          p_error=p_error, result=result)
                    if no_feature:
                        return data_size, p_error, no_feature
                else:
                    (data_size, p_error, coef, reg_data, result,
                     reg_store, selected_features) = PH.predict_subsets(
                                        data_size=data_size, p_error=p_error,
                                        result=result)
            if featselect_featvar:
                if int(index2) == 1:
                    PC.featselect_featvar_plot(p_error_select, number_feat)
