    ...
    def _concatenate_vec(self, atoms, vec_names):
        ...
        for name in vec_names:
            ...
            if len(fingerprint_vector) > 0:
                if ((arr.dtype.type is np.str_ and fingerprint_vector.dtype.type is not np.str_) or (arr.dtype.type is not np.str_ and fingerprint_vector.dtype.type is np.str_)):
