...
exit'''%(self.temp_path, self.magpie_path, self.txt_path, self.target, self.temp_path)
    ...
    def write_proto_input(self):
        ...
        for at in self.atoms:
            at_name=''
            for si in self.sites.site_list:
                at_name=at_name+getattr(at, si)+str(self.sites.site_dict[si][0])
    ...
    def run_proto(self):
        ...
        os.system("%s %s/prototype_FP.in |tee %s/prototype_FP.log"%(self.magpie, \
    ...
    def update_str(self):
        ...
        exit'''%(self.temp_path, self.magpie_path, self.txt_path, self.target, self.temp_path)
    def generate(self):
        '''Generate Prototype fingerprint and return all the fingerprint in pandas.Frame'''
        print('Generate Prototype fingerprint of %d structures'%len(self.atoms))
        self.update_str()
