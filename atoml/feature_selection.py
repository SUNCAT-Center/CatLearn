...
class feature_selection(object):
                    ...
                    if int_expand < 5:
                        feat_vec, alpha_vec, splits, int_expand =\
                           self.interval_modifier(feat_vec, alpha_vec,
                                                  feat, splits, int_expand)
                        feat_vec, alpha_vec, _ = self.feature_inspection(
                                                            upper=alpha_vec[0],
                                                            interval=splits)
                    else:
                        print("Warning feature", feat, "bad")
                        found_alpha = True
        return selected_features

    def feature_inspection(self, lower=0, upper=10**20, interval=10**2,
                           alpha_list=None):
    ...
    def interval_modifier(self, feat_vec, alpha_vec, feat, splits, int_expand):
        ...
        print(index)
        if index > 2:
            print("Remove", index - 3, "from vector")
