...
def dlogp(sigma, nfp, targets, regularization):
    """
    Routine to calculate the negative of the  log marginal likelyhood 
    as a function of the hyperparameters, the standardised or 
    normalized fingerprints and the target values.
    ...
    Output:
        -d/d(sigma_j) log(p), where logp is the the log marginal likelyhood.
    """
    # Set up the prediction routine.
    krr = FitnessPrediction(ktype='gaussian',
        kwidth=sigma,
    ...
    cov = krr.get_covariance(train_fp=nfp['train'])
    cinv = np.linalg.inv(cov)
    dcov = krr.get_derivatives(train_fp=nfp['train'])
    # Get the differential of log the marginal likelyhood
    dlogp = krr.dlog_marginal_likelyhood1(cinv,dcov,targets)
    return dlogp

def negative_dlogp(sigma, nfp, targets, regularization):
    """
    Routine to calculate the negative of the  log marginal likelyhood 
    as a function of the hyperparameters, the standardised or 
    normalized fingerprints and the target values.
    ...
    cov = krr.get_covariance(train_fp=nfp['train'])
    cinv = np.linalg.inv(cov)
    dK = krr.get_derivatives(train_fp=nfp['train'])
    # Get the differential of log the marginal likelyhood
    dlogp = krr.dlog_marginal_likelyhood1(cinv,dK,targets)
    return -dlogp