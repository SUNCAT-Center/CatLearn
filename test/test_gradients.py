...
from atoml.regression.gpfunctions.kernels import constant_kernel
import unittest
import numpy as np
...
class test_gaussian_kernel(unittest.TestCase):
    ...
    def test_cinv_is_good(self):
        k_dict = {'k1': {'type': 'gaussian', 'width': width, 'scaling':
        scaling}}
        gp = GaussianProcess(kernel_dict=k_dict, train_fp=train,
        train_target=target, gradients=None, scale_data=True,
        optimize_hyperparameters=False)
        gp_grad = GaussianProcess(kernel_dict=k_dict, train_fp=train,
        train_target=target, gradients=gradients, scale_data=True,
        optimize_hyperparameters=False)
        ...
        self.assertEqual(np.shape(gp_grad.cinv), (len(train) + len(
        train) * np.shape(train)[1], len(train) + len(train) * np.shape(train)[
        1]))
        ...
        np.testing.assert_array_almost_equal(cov, cov_grad[0:len(train),
        0:len(train)], decimal=10)
    ...
    def test_kernel_train(self):
        ...
        bigKgd_math = (width**(-2) * d * np.exp(-np.linalg.norm(
        d / width)**2 / 2))
        np.testing.assert_array_almost_equal(bigKgd_math, bigKgd[i_pos:i_pos
         + 1, j_pos * D:(j_pos + 1) * D][0], decimal=15)

        print('Checking bigKdd in bigKtilde.')
        bigKdd_math = (np.identity(len(d)) * width**(-2) - np.outer(width**(
        -2) * d, (width**(-2) * d).T)) * np.exp(-np.linalg.norm(d / width)**2
        / 2)
    ...
    def test_kernel_prediction(self):
        ...
        np.testing.assert_array_almost_equal(kgd_math, ktilde[i_pos,
        n_test + D * j_pos:(n_test + D * j_pos) + D], decimal=15)

    def test_hyperparameter_opt(self):
        k_dict = {'k1': {'type': 'gaussian', 'width': width, 'scaling':scaling}}
        gp_hyp = GaussianProcess(kernel_dict=k_dict, train_fp=train,
        train_target=target, gradients=gradients, scale_data=True,
