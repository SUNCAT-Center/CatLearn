...
print('Saving original',np.shape(fpm), 'matrix. Last column is the ase.db id. Second last column is the target value.')
np.savetxt('fpm.txt', fpm)

print('Creating combinatorial descriptors.')
order2 = get_order_2(cfpv)
L_F2 = get_labels_order_2(L_F)
fpm_ml = np.hstack([cfpv,order2])
L = np.hstack([L_F,L_F2])
assert np.isclose(len(L),np.shape(fpm_ml)[1])

print('Perfoming ISIS on '+str(np.shape(fpm_ml))+' combinatorial fp matrix.')
survivors=do_sis(fpm_ml, y, size=None, increment=2)
fmd_final=fpm_ml[:,survivors]
l=L[survivors]
...
fpm_y = np.hstack([fmd_final, np.vstack(y), np.vstack(dbid)])
