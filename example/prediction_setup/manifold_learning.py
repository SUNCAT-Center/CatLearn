...
def do_predict(train, test, train_target, test_target):
    ...
    gp = GaussianProcess(train_fp=std['train'], train_target=train_target,
                         kernel_dict=kdict, regularization=1e-4,
    ...
    print('Training error: {0:.3f}'.format(
        pred['training_error']['rmse_average']))
...
if new_data:
    hv.split_index(min_split=data_size, max_split=data_size*2)
...
print('Start SpectralEmbedding analysis')
...
tsne = manifold.TSNE(n_components=nc, random_state=0)
